(function() {
    ...
    // 4. Fetch and Update Data with Retry Logic
    let statsFetchRetries = 0;
    const MAX_STATS_RETRIES = 3;
    
    async function updateHeaderStats(retryCount = 0) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                // Retry on 500 errors
                if (response.status >= 500 && retryCount < MAX_STATS_RETRIES) {
                    const backoffMs = Math.min(1000 * Math.pow(2, retryCount), 5000);
                    console.warn(`[user-stats] Server error (${response.status}), retrying in ${backoffMs}ms (attempt ${retryCount + 1}/${MAX_STATS_RETRIES})`);
                    setTimeout(() => updateHeaderStats(retryCount + 1), backoffMs);
                    return;
                }
                console.error(`[user-stats] HTTP ${response.status}`, response.statusText);
                throw new Error(`HTTP ${response.status}`);
            }
            
            const data = await response.json();
            updateHeader(data);
            statsFetchRetries = 0; // Reset on success
            
        } catch (error) {
            console.error('[user-stats] Error fetching:', error.message);
            // Retry on network errors
            if (retryCount < MAX_STATS_RETRIES) {
                const backoffMs = Math.min(1000 * Math.pow(2, retryCount), 5000);
                console.warn(`[user-stats] Retrying in ${backoffMs}ms (attempt ${retryCount + 1}/${MAX_STATS_RETRIES})`);
                setTimeout(() => updateHeaderStats(retryCount + 1), backoffMs);
            }
        }
    }
    ...
    async function fetchNotifications(retryCount = 0) {
        try {
            ...
            if (!response.ok) {
                if (retryCount < 2) {
                    setTimeout(() => fetchNotifications(retryCount + 1), 1000 * Math.pow(2, retryCount));
                    return;
                }
                throw new Error(`HTTP ${response.status}`);
            }
