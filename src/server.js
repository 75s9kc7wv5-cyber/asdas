...
db.connect((err) => {
    if (err) {
        ...
        const createUserBusinessTable = `CREATE TABLE IF NOT EXISTS user_business (
            user_id INT NOT NULL PRIMARY KEY,
            rent_a_car_balance DECIMAL(15,2) DEFAULT 0,
            level INT DEFAULT 1,
            upgrade_end_time DATETIME,
            real_estate_balance DECIMAL(15,2) DEFAULT 0,
            real_estate_level INT DEFAULT 1,
            real_estate_upgrade_end_time DATETIME,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
        )`;
        
        db.query(createUserBusinessTable, (err) => {
            if (err) console.error('Error creating user_business table:', err);
            else console.log('user_business table ready');
        });

        // Create user_businesses Table (Ownership tracking)
        const createUserBusinessesTable = `CREATE TABLE IF NOT EXISTS user_businesses (
            id INT AUTO_INCREMENT PRIMARY KEY,
            user_id INT NOT NULL,
            business_type VARCHAR(50) NOT NULL,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            UNIQUE KEY unique_user_biz (user_id, business_type),
            FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
        )`;
        
        db.query(createUserBusinessesTable, (err) => {
            if (err) console.error('Error creating user_businesses table:', err);
            else console.log('user_businesses table ready');
        });

        // Create user_garage Table
        const createUserGarageTable = `CREATE TABLE IF NOT EXISTS user_garage (
            id INT AUTO_INCREMENT PRIMARY KEY,
            user_id INT NOT NULL,
        ...
        db.query(createUserGarageTable, (err) => {
            if (err) console.error('Error creating user_garage table:', err);
            else console.log('user_garage table ready');
        });

        // Create property_types Table
        const createPropertyTypesTable = `CREATE TABLE IF NOT EXISTS property_types (
            id INT AUTO_INCREMENT PRIMARY KEY,
        ...
        db.query(createPropertyTypesTable, (err) => {
            if (err) console.error('Error creating property_types table:', err);
            else console.log('property_types table ready');
        });

        // Create user_properties Table
        const createUserPropertiesTable = `CREATE TABLE IF NOT EXISTS user_properties (
            id INT AUTO_INCREMENT PRIMARY KEY,
            user_id INT NOT NULL,
        ...
        db.query(createUserPropertiesTable, (err) => {
            if (err) console.error('Error creating user_properties table:', err);
            else console.log('user_properties table ready');
        });

        // Create factory_inventory Table
        const createFactoryInventoryTable = `CREATE TABLE IF NOT EXISTS factory_inventory (
            id INT AUTO_INCREMENT PRIMARY KEY,
            mine_id INT NOT NULL,
            item_key VARCHAR(100) NOT NULL,
            amount INT DEFAULT 0,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            UNIQUE KEY unique_inventory (mine_id, item_key),
            FOREIGN KEY (mine_id) REFERENCES player_mines(id) ON DELETE CASCADE
        )`;
        
        db.query(createFactoryInventoryTable, (err) => {
            if (err) console.error('Error creating factory_inventory table:', err);
            else console.log('factory_inventory table ready');
        });

        // Create mine_settings Table (if doesn't exist)
        const createMineSettingsTable = `CREATE TABLE IF NOT EXISTS mine_settings (
            id INT AUTO_INCREMENT PRIMARY KEY,
            mine_type VARCHAR(100) NOT NULL UNIQUE,
            production_time INT DEFAULT 60,
            salary INT DEFAULT 100,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        )`;
        
        db.query(createMineSettingsTable, (err) => {
            if (err) console.error('Error creating mine_settings table:', err);
            else console.log('mine_settings table ready');
        });

        // Create arge_levels Table (if doesn't exist)
        const createArgeLevelsTable = `CREATE TABLE IF NOT EXISTS arge_levels (
            id INT AUTO_INCREMENT PRIMARY KEY,
            user_id INT NOT NULL,
            mine_type VARCHAR(100) NOT NULL,
            level INT DEFAULT 0,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            UNIQUE KEY unique_user_arge (user_id, mine_type),
            FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
        )`;
        
        db.query(createArgeLevelsTable, (err) => {
            if (err) console.error('Error creating arge_levels table:', err);
            else console.log('arge_levels table ready');
            
            // Add crypto_slots column if not exists
            const addCryptoSlotsColumn = `ALTER TABLE users ADD COLUMN crypto_slots INT DEFAULT 1`;
            db.query(addCryptoSlotsColumn, (err) => {
                if (err && !err.message.includes('Duplicate column')) {
                    console.error('Error adding crypto_slots column:', err);
                } else if (!err) {
                    console.log('crypto_slots column added');
                }
            });
        });
    }
...
app.use((req, res, next) => {
    ...
    res.setHeader('Content-Security-Policy', "default-src 'none'; script-src 'self' 'unsafe-inline' https://code.jquery.com/jquery-3.6.0.min.js https://cdnjs.cloudflare.com https://cdn.jsdelivr.net https://static.cloudflareinsights.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://cdnjs.cloudflare.com; font-src 'self' data: https://*.googleapis.com https://*.gstatic.com https://fonts.googleapis.com https://fonts.gstatic.com https://cdnjs.cloudflare.com; img-src 'self' data: blob: https://upload.wikimedia.org https://images.unsplash.com; connect-src 'self' https://cdnjs.cloudflare.com https://cdn.jsdelivr.net https://cloudflareinsights.com; frame-ancestors 'none'; base-uri 'self'; form-action 'self';" );
    ...
    // Permissions-Policy - İzinleri kısıtla
...
app.get('/api/inventory/:userId', (req, res) => {
    ...
    db.query(createTableQuery, (err) => {
        if (err) console.error('Inventory table creation error:', err);
...
app.get('/api/items', (req, res) => {
    const query = 'SELECT * FROM items ORDER BY sort_order ASC';
    db.query(query, (err, results) => {
...
app.get('/api/user-stats/:userId', (req, res) => {
    ...
    db.query(checkTreatmentQuery, [userId], (err, finishedTreatments) => {
        ...
        db.query(activeTreatmentQuery, [userId], (err, activeTreatments) => {
            ...
            const query = 'SELECT money, gold, diamond, health, energy, level, license_hospital_level, license_farm_level, license_property_level, education_skill, avatar, username, party_id, role FROM users WHERE id = ?';
            db.query(query, [userId], (err, results) => {
                if (err) return res.status(500).json({ error: err });
                ...
                db.query('SELECT * FROM licenses WHERE user_id = ? AND mine_type = "political_party"', [userId], (err, licenses) => {
                    ...
                    // Fetch Inventory Items (Construction Materials + Gold)
                    const invQuery = "SELECT item_key, quantity FROM inventory WHERE user_id = ? AND item_key IN ('lumber', 'brick', 'concrete', 'glass', 'steel', 'gold')";
                    db.query(invQuery, [userId], (err, invResults) => {
                        ...
                        userData.glass = 0;
                        userData.steel = 0;
                        // Keep users.gold as fallback, but override with inventory.gold if exists
                        let inventoryGold = null;
                        ...
                        if (!err && invResults.length > 0) {
                            invResults.forEach(row => {
                                ...
                                if(row.item_key === 'gold') inventoryGold = row.quantity;
                            });
                        }
                        
                        // Override gold with inventory value if exists
                        if (inventoryGold !== null) {
                            userData.gold = inventoryGold;
                        }
...
app.post('/api/parties/create', (req, res) => {
    ...
    color = '#e74c3c';
        ...
        // Check user
        db.query('SELECT money, gold, diamond, party_id, username FROM users WHERE id = ?', [userId], (err, users) => {
            ...
            // Ensure user resources are numbers
            // Fix: Use loose check or explicit casting, handle potential nulls
            const userMoney = user.money ? parseFloat(user.money) : 0;
            const userGold = user.gold ? parseInt(user.gold) : 0;
            const userDiamond = user.diamond ? parseInt(user.diamond) : 0;
    
            if (userMoney < costMoney) return res.json({ success: false, message: `Yetersiz bakiye (Para). Mevcut: ${userMoney.toLocaleString()}, Gerekli: ${costMoney.toLocaleString()}` });
            if (userGold < costGold) return res.json({ success: false, message: 'Yetersiz bakiye (Altın).' });
            if (userDiamond < costDiamond) return res.json({ success: false, message: 'Yetersiz bakiye (Elmas).' });
    
            // Check license
            db.query('SELECT * FROM licenses WHERE user_id = ? AND mine_type = "political_party"', [userId], (err, licenses) => {
                if (err) return res.status(500).json({ success: false, message: 'Lisans kontrol hatası.' });
                // Disabled license check for now or ensure data is correct? 
                // The user did not complain about license, but let's keep it.
                ...
                db.beginTransaction(err => {
                    ...
                    db.query(createQuery, [name, abbr, userId, user.username, ideology, color], (err, result) => {
                        if (err) return db.rollback(() => res.status(500).json({ success: false, message: 'Parti oluşturulamadı: ' + err.message }));
                        
                        const partyId = result.insertId;
                        
                        // Update User (Money, Gold, Diamond & Party ID)
                        // Verify balance condition in WHERE clause to be atomic
                        const updateUser = 'UPDATE users SET money = money - ?, gold = gold - ?, diamond = diamond - ?, party_id = ? WHERE id = ? AND money >= ? AND gold >= ? AND diamond >= ?';
                        db.query(updateUser, [costMoney, costGold, costDiamond, partyId, userId, costMoney, costGold, costDiamond], (err, updateResult) => {
                            if (err) return db.rollback(() => res.status(500).json({ success: false, message: 'Kullanıcı güncellenemedi.' }));
                            
                            if (updateResult.affectedRows === 0) {
                                // This means either user not found OR insufficient funds (condition failed)
                                return db.rollback(() => res.json({ success: false, message: 'Yetersiz bakiye veya kullanıcı hatası.' }));
                            }
...
const FACTORY_RECIPES = {
    ...
    'gold_factory': [
        { id: 'gold_ingot', name: 'Altın', inputs: { gold_nugget: 30, electricity: 1 }, output: { gold: 1 } }
    ],
    'silicon': [
        { id: 'silicon', name: 'Silikon', inputs: { sand: 3, coal: 1, electricity: 1 }, output: { silicon: 1 } }
    ],
    'plastic_factory': [
        { id: 'plastic', name: 'Plastik', inputs: { oil: 3, electricity: 1 }, output: { plastic: 1 } }
    ],
    'chip_factory': [
        { id: 'chip', name: 'Çip', inputs: { silicon: 3, gold: 1, copper: 1, electricity: 1 }, output: { chip: 1 } }
    ],
    'tyre_factory': [
        { id: 'tire', name: 'Lastik', inputs: { oil: 3, electricity: 1 }, output: { tire: 1 } }
    ],
    'engine_factory': [
        { id: 'engine', name: 'Motor', inputs: { steel: 3, chip: 1, electronic_card: 1, gold_cable: 3, copper_cable: 3, electricity: 1 }, output: { engine: 1 } }
    ],
    'cable_factory': [
    ...
    'electronic_card_factory': [
        { id: 'electronic_card', name: 'Elektronik Kart', inputs: { silicon: 1, plastic: 3, copper_cable: 1, gold_cable: 1, electricity: 1 }, output: { electronic_card: 1 } }
    ],
    'gpu_factory': [
    ...
    'car_factory': [
        { id: 'noventis', name: 'Noventis', inputs: { steel: 2, glass: 4, engine: 1, tire: 4, electricity: 1 }, output: { noventis: 1 } },
        { id: 'stradeo', name: 'Stradeo', inputs: { steel: 4, glass: 4, engine: 1, tire: 4, electricity: 2 }, output: { stradeo: 1 } },
        { id: 'rugnar', name: 'Rugnar', inputs: { steel: 6, glass: 4, engine: 1, tire: 4, electricity: 3 }, output: { rugnar: 1 } },
        { id: 'veltrano', name: 'Veltrano', inputs: { steel: 8, glass: 2, engine: 2, tire: 4, electricity: 4 }, output: { veltrano: 1 } },
        { id: 'zentaro', name: 'Zentaro', inputs: { steel: 10, glass: 2, engine: 2, tire: 4, electricity: 5 }, output: { zentaro: 1 } }
    ],
    'coal_plant': [
...
const FACTORY_INPUTS = {
    ...
    'tyre_factory': ['oil', 'electricity'],
    'engine_factory': ['steel', 'chip', 'electronic_card', 'gold_cable', 'copper_cable', 'electricity'],
    'car_factory': ['steel', 'plastic', 'tire', 'engine', 'electricity'],
    'cable_factory': ['copper', 'plastic', 'gold', 'electricity'],
    'electronic_card_factory': ['silicon', 'plastic', 'copper_cable', 'gold_cable', 'electricity'],
    'chip_factory': ['silicon', 'gold', 'copper', 'electricity'],
    'gpu_factory': ['chip', 'electronic_card', 'plastic', 'copper_cable', 'gold_cable', 'silicon', 'electricity']
};
...
app.get('/api/mines/detail/:id', (req, res) => {
    ...
    db.query('DELETE FROM mine_active_workers WHERE end_time < DATE_SUB(NOW(), INTERVAL 1 HOUR)', (err) => {
        ...
        const query = `
            SELECT m.id, m.name, m.user_id, m.mine_type, m.level, m.reserve, m.salary, m.vault, m.stock, m.workers, m.max_workers, m.raw_material, m.raw_material_2, m.raw_capacity, m.product_capacity, m.active_recipe_id, m.created_at, u.username as owner_username
            FROM player_mines m 
        ...
        db.query(query, [mineId], (err, results) => {
            if (err) {
                console.error('DB Error in mine detail - Main Query:', err);
                return res.status(500).json({ success: false, message: `DB Error: ${err.message}` });
            }
            
            if (results.length === 0) {
                return res.status(404).json({ success: false, message: 'Maden bulunamadı.' });
            }
            
            const mine = results[0];
            mine.owner_id = mine.user_id;
            mine.owner_name = mine.owner_username;
            mine.production_time = mine.production_time || 60; // Default production time
                ...
                db.query(argeQuery, [mine.owner_id, mine.mine_type], (err, argeRes) => {
                    ...
                    db.query(logQuery, [mineId], (err, logs) => {
                        ...
                        db.query(transLogQuery, [mineId], (err, transactionLogs) => {
                            if (err) {
                                console.error('Trans Logs Error:', err);
                                transactionLogs = [];
                            }
                            ...
                            db.query('SELECT item_key, amount FROM factory_inventory WHERE mine_id = ?', [mineId], (err, invRes) => {
                                ...
                                // Get Active Workers with remaining time
                                const workersQuery = `
                                ...
                                db.query(workersQuery, [mineId, userId], (err, workers) => {
                                    ...
                                    mine.efficiency = Math.min(30 + (argeLevel * 10), 100);

                                    res.json({ success: true, mine, logs: logs || [], transactionLogs: transactionLogs || [], workers: workers || [] });
                                });
                            });
                        });
                    });
...
// Update Factory Recipe
app.post('/api/factory/set-recipe', (req, res) => {
    const { factoryId, recipeId, userId } = req.body;
    
    // Validate inputs
    if (!factoryId || !recipeId) {
        return res.status(400).json({ success: false, message: 'Geçersiz parametreler.' });
    }

    db.query('UPDATE player_mines SET active_recipe_id = ? WHERE id = ?', [recipeId, factoryId], (err, result) => {
        if (err) {
            console.error('Factory Recipe Update Error:', err);
            return res.status(500).json({ success: false, message: 'Sunucu hatası.' });
        }
            ...
            if (itemKey) {
                db.query('SELECT amount FROM factory_inventory WHERE mine_id = ? AND item_key = ?', [mineId, itemKey], (err, results) => {
                    ...
                    if (factoryStock >= withdrawAmount) {
                        ...
                        if (mine.mine_type === 'gold_factory' && itemKey === 'gold') isLegacyMatch = true; // Gold factory legacy support
...
app.post('/api/mines/deposit-raw/:id', (req, res) => {
    ...
    // DEBUG: Response'a eklenecek
    const debugInfo = {
        enteredEndpoint: true,
        mineId, userId, amount, itemKey
    };
    ...
    db.beginTransaction(err => {
        ...
        db.query('SELECT * FROM player_mines WHERE id = ?', [mineId], (err, mines) => {
            ...
            debugInfo.mineFound = true;
            debugInfo.mineType = mine.mine_type;

            // Check if it's a new factory type or new input for legacy factory
            const allowedInputs = FACTORY_INPUTS[mine.mine_type];
            debugInfo.allowedInputs = allowedInputs;
            debugInfo.includesCheck = allowedInputs && allowedInputs.includes(itemKey);
                ...
                db.query('SELECT quantity FROM inventory WHERE user_id = ? AND item_key = ?', [userId, itemKey], (err, inv) => {
                    ...
                    db.query('UPDATE inventory SET quantity = quantity - ? WHERE user_id = ? AND item_key = ?', [amount, userId, itemKey], (err) => {
                        if (err) {
                            ...
                            return db.rollback(() => res.status(500).json({ success: false, message: 'Envanter güncellenemedi.' }));
                        }
                        console.log('✅ Deducted', amount, itemKey, 'from user', userId);
                        ...
                        `;
                        db.query(upsert, [mineId, itemKey, amount, amount], (err) => {
                            if (err) {
                                console.error('❌ Factory inventory update error:', err);
                                return db.rollback(() => res.status(500).json({ success: false, message: 'Fabrika deposu güncellenemedi.' }));
                            }
                            console.log('✅ Added', amount, itemKey, 'to factory', mineId);
                            ...
                            db.commit(err => {
                                ...
                                res.json({ success: true, message: `${amount} adet ${itemKey} fabrikaya eklendi.`, debug: debugInfo });
            ...
            // LEGACY SYSTEM - Only for factories NOT in FACTORY_INPUTS
            // lumber, brick, glass, concrete, steel are now in new system - removed from here
            if (mine.mine_type === 'concrete_old_legacy_removed') {
                // Concrete moved to new system
            ...
            } else if (mine.mine_type === 'steel_old_legacy_removed') {
                // Steel moved to new system
                if (requiredItemKey === 'iron') { validItem = true; targetColumn = 'raw_material'; }
                 ...
                 // No legacy factories left - all should use new system
                 return db.rollback(() => res.json({ success: false, message: 'Bu fabrika tipi yeni sistem kullanmalı (FACTORY_INPUTS).' }));
            }
            ...
            db.query('SELECT quantity FROM inventory WHERE user_id = ? AND item_key = ?', [userId, requiredItemKey], (err, inv) => {
                ...
                db.query('UPDATE inventory SET quantity = quantity - ? WHERE user_id = ? AND item_key = ?', [amount, userId, requiredItemKey], (err) => {
                    ...
                    db.query(updateQuery, [amount, mineId], (err) => {
                        ...
                        db.commit(err => {
                            if (err) return db.rollback(() => res.status(500).json({ success: false, message: 'Commit Error' }));
                            debugInfo.usingLegacySystem = true;
                            debugInfo.legacyColumn = targetColumn;
                            res.json({ success: true, message: `${amount} adet ${requiredItemKey} eklendi.`, debug: debugInfo });
                        });
                    });
                });
            });
...
app.post('/api/admin/add-item', (req, res) => {
    ...
    db.beginTransaction(err => {
        ...
        db.query('SELECT username FROM users WHERE id = ?', [userId], (err, users) => {
            if (err || users.length === 0) return db.rollback(() => res.json({ success: false, message: 'Kullanıcı bulunamadı.' }));
            ...
            db.query(query, [userId, itemKey, qty], (err) => {
                if (err) return db.rollback(() => res.status(500).json({ success: false, message: 'Envanter güncellenemedi.' }));

                // Log Action
                const logMsg = `Admin tarafından ${qty} adet ${itemKey} eklendi.`;
                db.query('INSERT INTO user_logs (user_id, log_type, message) VALUES (?, ?, ?)', [userId, 'admin_add_item', logMsg], (err) => {
...
app.post('/api/mines/start', (req, res) => {
    ...
    db.beginTransaction(err => {
        ...
        db.query(query, [userId, userId, mineId], (err, results) => {
            ...
            // Raw Material Check (Only for Factories)
            // Factories consume raw materials to produce goods.
            // Assuming 'mine_type' distinguishes factories.
            // If it's a factory (e.g. lumber mill), check for raw material (wood).
            // For now, let's assume all player_mines are factories that need raw materials EXCEPT mines (gold, coal etc which extract from reserve).
            // But the prompt says "3 odun harcanıp 1 tahta elde edilecek". This implies a conversion.
            // Let's check if raw_material column exists and is sufficient.
            ...
            db.query('SELECT item_key, amount FROM factory_inventory WHERE mine_id = ?', [mineId], (err, invRes) => {
                 ...
                 // Note: raw_material and raw_material_2 columns don't exist in player_mines
                 // We'll just use the inventory system
                 const legacyRaw1 = 0;
                 const legacyRaw2 = 0;
                 ...
                 db.query('SELECT education_skill FROM users WHERE id = ?', [data.owner_id], (err, ownerData) => {
                     ...
                     // GPU Factory Override: Always 1 unit
                     if (data.mine_type === 'gpu_factory' || data.mine_type === 'car_factory' || data.mine_type === 'gold_factory') {
                         productionAmount = 1;
                     }
                     ...
                     if (recipes) {
                         const recipeId = data.active_recipe_id || req.body.recipeId;
                     ...
                     // Default for non-factory mines (wood, stone, etc.)
    ...
    db.beginTransaction(err => {
        ...
        db.query('SELECT * FROM mine_active_workers WHERE user_id = ? AND mine_id = ?', [userId, mineId], (err, workers) => {
            ...
            db.query(query, [userId, mineId], (err, results) => {
                ...
                let amount = worker.amount || 1;
                
                // FORCE OVERRIDE: Double check for single-unit factories during collection
                if (data.mine_type === 'gpu_factory' || data.mine_type === 'car_factory' || data.mine_type === 'gold_factory') {
                    amount = 1;
                    console.log(`[Mining Collect] Forced amount to 1 for ${data.mine_type}`);
                }
                ...
                db.query('UPDATE users SET money = money + ? WHERE id = ?', [userMoneyChange, userId], (err) => {
                    ...
                    db.query(updateStockQuery, updateStockParams, (err) => {
                        ...
                        db.query('UPDATE player_mines SET vault = vault + ? WHERE id = ?', [vaultChange, mineId], (err) => {
                            ...
                            // Log Production (Use NULL log_type so it appears in production history)
                            const logQuery = 'INSERT INTO mine_logs (mine_id, user_id, amount, earnings, product_key, log_type) VALUES (?, ?, ?, ?, ?, NULL)';
...
app.post('/api/ranches/start', (req, res) => {
    ...
    db.beginTransaction(err => {
        ...
        db.query(query, [userId, userId, ranchId], (err, results) => {
            if (err) return db.rollback(() => res.status(500).json({ success: false, message: 'DB Error' }));
            ...
            if (productionAmount <= 0) {
                return db.rollback(() => res.json({ success: false, message: 'Yetersiz Yem (Rezerv)!' }));
            }
            ...
            if (current_workers >= (max_workers || 5)) {
                ...
                db.query('UPDATE player_ranches SET reserve = reserve - ? WHERE id = ?', [totalFeedCost, ranchId], (err) => {
                    ...
                    db.query('INSERT INTO ranch_active_workers (ranch_id, user_id, end_time, amount) VALUES (?, ?, ?, ?)', [ranchId, userId, endTime, productionAmount], (err) => {
                        if (err) return db.rollback(() => res.status(500).json({ success: false, message: 'Active Worker Error' }));
...
app.post('/api/ranches/collect', (req, res) => {
    ...
    db.beginTransaction(err => {
        ...
        // 1. Check Active Worker
        db.query('SELECT * FROM ranch_active_workers WHERE user_id = ? AND ranch_id = ?', [userId, ranchId], (err, workers) => {
            if (err) return db.rollback(() => res.status(500).json({ success: false, message: 'DB Error' }));
            if (workers.length === 0) return db.rollback(() => res.json({ success: false, message: 'Aktif çalışma bulunamadı.' }));
            ...
            if (new Date() < new Date(worker.end_time)) {
                return db.rollback(() => res.json({ success: false, message: 'İş henüz bitmedi.' }));
            }
            ...
            db.query('SELECT * FROM player_ranches WHERE id = ?', [ranchId], (err, ranches) => {
                ...
                db.query('UPDATE users SET money = money + ? WHERE id = ?', [totalEarnings, userId], (err) => {
                    if (err) return db.rollback(() => res.status(500).json({ success: false, message: 'User Money Error' }));

                    // 4. Update Ranch (Stock +, Vault -)
                    db.query('UPDATE player_ranches SET stock = stock + ?, vault = vault - ? WHERE id = ?', 
                        ...
                        db.query(logQuery, [ranchId, userId, message, amount], (err) => {
                            ...
                            // Remove Active Worker
                            db.query('DELETE FROM ranch_active_workers WHERE id = ?', [worker.id], (err) => {
                                if (err) return db.rollback(() => res.status(500).json({ success: false, message: 'Delete Worker Error' }));
...
app.get('/api/farms/detail/:id', (req, res) => {
    ...
    db.query('DELETE FROM farm_active_workers WHERE end_time < DATE_SUB(NOW(), INTERVAL 1 HOUR)', (err) => {
        ...
        const query = `
            SELECT pf.*, ft.name as farm_name, ft.name as name, ft.slug as farm_type, ft.image_path, 
            u.username as owner_name, pf.user_id as owner_id,
            60 as production_time
            FROM player_farms pf 
            JOIN farm_types ft ON pf.farm_type_id = ft.id
...
app.post('/api/farms/start', (req, res) => {
    ...
    db.beginTransaction(err => {
        ...
        db.query(query, [userId, userId, farmId], (err, results) => {
            if (err) return db.rollback(() => res.status(500).json({ success: false, message: 'DB Error' }));
...
app.post('/api/farms/collect', (req, res) => {
    ...
    db.beginTransaction(err => {
        ...
        // 1. Check Active Worker
        db.query('SELECT * FROM farm_active_workers WHERE user_id = ? AND farm_id = ?', [userId, farmId], (err, workers) => {
            if (err) return db.rollback(() => res.status(500).json({ success: false, message: 'DB Error' }));
            if (workers.length === 0) return db.rollback(() => res.json({ success: false, message: 'Aktif çalışma bulunamadı.' }));
            ...
            // 2. Get Farm Info
            db.query('SELECT pf.*, ft.slug as farm_type_slug FROM player_farms pf JOIN farm_types ft ON pf.farm_type_id = ft.id WHERE pf.id = ?', [farmId], (err, farms) => {
                ...
                db.query('UPDATE users SET money = money + ? WHERE id = ?', [totalEarnings, userId], (err) => {
                    ...
                    db.query('UPDATE player_farms SET stock = stock + ?, vault = vault - ? WHERE id = ?', 
...
app.post('/api/farms/deposit-seed/:id', (req, res) => {
    ...
    db.beginTransaction(err => {
        ...
        db.query('SELECT * FROM player_farms WHERE id = ?', [farmId], (err, farms) => {
            ...
            db.query('SELECT quantity FROM inventory WHERE user_id = ? AND item_key = ?', [userId, seedKey], (err, inv) => {
                if (err) return db.rollback(() => res.status(500).json({ success: false, message: 'Envanter hatası.' }));
                
                const userQty = (inv && inv.length > 0) ? inv[0].quantity : 0;
                if (userQty < amount) {
                    return db.rollback(() => res.json({ success: false, message: `Envanterde yeterli ${seedKey} yok.` }));
                }

                // Deduct from Inventory
                db.query('UPDATE inventory SET quantity = quantity - ? WHERE user_id = ? AND item_key = ?', [amount, userId, seedKey], (err) => {
                    if (err) return db.rollback(() => res.status(500).json({ success: false, message: 'Envanter güncellenemedi.' }));

                    // Add to Farm Reserve
                    db.query('UPDATE player_farms SET reserve = reserve + ? WHERE id = ?', [amount, farmId], (err) => {
                        if (err) return db.rollback(() => res.status(500).json({ success: false, message: 'Tarla deposu güncellenemedi.' }));
...
setInterval(() => {
    ...
    db.query(query, (err, results) => {
        ...
        results.forEach(job => {
            ...
            db.beginTransaction(err => {
                if (err) return console.error('Auto-Collect Transaction Error:', err);

                // 1. Pay User
                db.query('UPDATE users SET money = money + ? WHERE id = ?', [totalEarnings, job.user_id], (err) => {
                    if (err) return db.rollback(() => console.error('Auto-Collect User Update Error:', err));
                    ...
                    db.query(updateStockQuery, updateStockParams, (err) => {
                        if (err) return db.rollback(() => console.error('Auto-Collect Stock Update Error:', err));

                        // Update Vault
                        db.query('UPDATE player_mines SET vault = vault - ? WHERE id = ?', [totalEarnings, job.mine_id], (err) => {
                            if (err) return db.rollback(() => console.error('Auto-Collect Vault Update Error:', err));

                            // 3. Log
                            const logQuery = 'INSERT INTO mine_logs (mine_id, user_id, amount, earnings, product_key, log_type) VALUES (?, ?, ?, ?, ?, NULL)';
                            db.query(logQuery, [job.mine_id, job.user_id, amount, totalEarnings, productKey], (err) => {
                                if (err) return db.rollback(() => console.error('Auto-Collect Log Error:', err));

                                // 4. Delete Worker
                                db.query('DELETE FROM mine_active_workers WHERE id = ?', [job.worker_id], (err) => {
                                    if (err) return db.rollback(() => console.error('Auto-Collect Delete Error:', err));

                                    // 5. Notification
                                    const notifTitle = 'Üretim Tamamlandı';
                                    const notifMsg = `Otomatik toplama: +${amount} ${productKey || 'Ürün'}, +${totalEarnings} ₺ Kazanç.`;
                                    db.query('INSERT INTO notifications (user_id, title, message, type) VALUES (?, ?, ?, ?)', [job.user_id, notifTitle, notifMsg, 'production'], (err) => {
                                        
                                        db.commit(err => {
                                            if (err) return db.rollback(() => console.error('Auto-Collect Commit Error:', err));
                                            // Success
                                        });
                                    });
                                });
...
app.post('/api/buy-farm', (req, res) => {
    ...
    db.query('SELECT * FROM farm_types WHERE id = ?', [farm_type_id], (err, types) => {
        ...
        db.query('SELECT money, gold, diamond, license_farm_level, username FROM users WHERE id = ?', [user_id], (err, users) => {
            if (err) return res.status(500).json({ message: 'Veritabanı hatası' });
            ...
            db.query('SELECT level FROM licenses WHERE user_id = ? AND mine_type = ?', [user_id, farmType.slug], (err, licRes) => {
                ...
                db.beginTransaction(err => {
                    ...
                    db.query('UPDATE users SET money = money - ?, gold = gold - ?, diamond = diamond - ? WHERE id = ?', 
                        ...
                        // Add Farm
                        // Removed name column as it does not exist in player_farms table
                        db.query('INSERT INTO player_farms (user_id, farm_type_id) VALUES (?, ?)', [user_id, farm_type_id], (err) => {
                            if (err) {
                                console.error(err);
                                return db.rollback(() => res.status(500).json({ message: 'Çiftlik eklenemedi' }));
                            }
...
app.post('/api/ranches/deposit-feed/:id', (req, res) => {
    const ranchId = req.params.id;
    const { userId, amount, itemKey } = req.body; // itemKey should be 'feed'
    ...
    db.beginTransaction(err => {
        ...
        db.query('SELECT * FROM player_ranches WHERE id = ?', [ranchId], (err, ranches) => {
            ...
            // Check Capacity (Reserve Capacity)
            // Assuming reserve capacity is level * 10000 (as defined in frontend)
            ...
            db.query('SELECT quantity FROM inventory WHERE user_id = ? AND item_key = ?', [userId, feedKey], (err, inv) => {
                if (err) return db.rollback(() => res.status(500).json({ success: false, message: 'Envanter hatası.' }));
                
                const userQty = (inv && inv.length > 0) ? inv[0].quantity : 0;
                if (userQty < amount) {
                    return db.rollback(() => res.json({ success: false, message: `Envanterde yeterli ${feedKey} yok.` }));
                }

                // Deduct from Inventory
                db.query('UPDATE inventory SET quantity = quantity - ? WHERE user_id = ? AND item_key = ?', [amount, userId, feedKey], (err) => {
                    if (err) return db.rollback(() => res.status(500).json({ success: false, message: 'Envanter güncellenemedi.' }));

                    // Add to Ranch Reserve
                    db.query('UPDATE player_ranches SET reserve = reserve + ? WHERE id = ?', [amount, ranchId], (err) => {
                        if (err) return db.rollback(() => res.status(500).json({ success: false, message: 'Çiftlik deposu güncellenemedi.' }));

                        db.commit(err => {
                            if (err) return db.rollback(() => res.status(500).json({ success: false, message: 'Commit Error' }));
                            res.json({ success: true, message: `${amount} adet yem eklendi.` });
                        });
                    });
                });
            });
...
app.get('/api/city-farms', (req, res) => {
    const query = `
        SELECT pf.*, ft.name as type_name, ft.slug, ft.image_path, u.username
        FROM player_farms pf
        JOIN farm_types ft ON pf.farm_type_id = ft.id
        JOIN users u ON pf.user_id = u.id
        ORDER BY pf.created_at DESC
...
app.get('/api/ranches/city', (req, res) => {
    const query = `
        SELECT pr.id as ranch_id, pr.*, rt.name as type_name, rt.slug as type, rt.image_path, u.username
        FROM player_ranches pr
        JOIN ranch_types rt ON pr.ranch_type_id = rt.id
        JOIN users u ON pr.user_id = u.id
        ORDER BY pr.created_at DESC
            ...
            if (type === 'buy') {
                ...
                db.beginTransaction(err => {
                    ...
                    db.query('UPDATE users SET money = money - ?, btc = btc + ?, last_crypto_trade = NOW() WHERE id = ?', 
                        [Math.floor(totalCost), val, userId], (err) => {
                        ...
                        db.query('INSERT INTO market_trades (user_id, type, price, amount, total_price) VALUES (?, ?, ?, ?, ?)',
                            [userId, 'buy', price, val, Math.floor(totalCost)], (err) => {
                 ...
                 db.beginTransaction(err => {
                    ...
                    db.query('UPDATE users SET money = money + ?, btc = btc - ?, last_crypto_trade = NOW() WHERE id = ?', 
                        ...
                        // Log Trade
                        db.query('INSERT INTO market_trades (user_id, type, price, amount, total_price) VALUES (?, ?, ?, ?, ?)',
                            [userId, 'sell', price, val, Math.floor(totalCost)], (err) => {
                            
                            db.commit(err => {
                                if(err) return db.rollback(() => res.json({ success: false, message: 'Commit Error' }));
                                res.json({ success: true, message: `Satış Başarılı: +$${Math.floor(totalCost)}` });
                            });
                        });
                    });
                });
...
setInterval(() => {
    ...
    db.query('SELECT * FROM player_rigs WHERE active = 1 AND health > 0', (err, rigs) => {
        ...
        rigs.forEach(rig => {
            // Check GPU Stats
            // Note: DB stores 'gpu_key' or 'gpu' depending on schema. 
            // Previous code used 'gpu_key'. Let's check schema/previous usage.
            // In crypto-mining.html it uses 'rig.gpu'. In install endpoint it uses 'gpuKey'.
            // Let's assume the column is 'gpu_key' or 'gpu'. 
            // Based on previous code snippet: 'const stats = GPU_STATS[rig.gpu_key];'
            // I'll stick with gpu_key but fallback to gpu just in case.
            const gpuKey = rig.gpu_key || rig.gpu;
            const stats = GPU_STATS[gpuKey];
            
            if(!stats) {
                console.error("[Mining] Unknown GPU:", gpuKey);
                return;
            }

            // Time difference since last update
            // Fallback to installed_at or now if last_collection is null
            const lastTime = new Date(rig.last_collection || rig.installed_at || now);
            const diffMs = now - lastTime;
            
            // Skip processing if less than 10 seconds passed (prevents rapid-fire updates)
            ...
            // Calculate Health Loss (approx 1% every 8 hours -> ~3%/day)
            // 100% health lasts ~33 days
            const healthLoss = 3.0 * diffDays; 

            // Update user balance and rig status
            db.beginTransaction(err => {
                if(err) return console.error("[Mining] Tx Error", err);

                // 1. Add BTC to user
                db.query('UPDATE users SET btc = btc + ? WHERE id = ?', [earnings, rig.user_id], (err) => {
                    if(err) return db.rollback(() => console.error("[Mining] User Update Error", err));

                    // 2. Update Rig (Health & Last Collection)
                    db.query('UPDATE player_rigs SET health = GREATEST(0, health - ?), last_collection = NOW() WHERE id = ?', 
                    [healthLoss, rig.id], (err) => {
                        if(err) return db.rollback(() => console.error("[Mining] Rig Update Error", err));
...
app.post('/api/business/rent-a-car/withdraw', (req, res) => {
    ...
    db.query('SELECT rent_a_car_balance FROM user_business WHERE user_id = ?', [userId], (err, rows) => {
        ...
        let amount = parseInt(rows[0].rent_a_car_balance || 0);
        ...
        db.query('UPDATE user_business SET rent_a_car_balance = 0 WHERE user_id = ?', [userId], (err) => {
            ...
            db.query('UPDATE users SET money = money + ? WHERE id = ?', [amount, userId], (err) => {
                res.json({ success: true, message: `${amount.toLocaleString()} TL ana kasanıza aktarıldı.` });
            });
        });
    });
});
    ...
    db.query('INSERT IGNORE INTO user_business (user_id) VALUES (?)', [userId], (err) => {
        ...
        const sql = `
            SELECT b.real_estate_balance, b.real_estate_level, b.real_estate_upgrade_end_time,
                   ...
                   pt.name, pt.image, pt.tax_income as income, pt.tax_interval, pt.cost_money as price,
        ...
        db.query(sql, [userId], (err, results) => {
            ...
            db.query('SELECT * FROM user_businesses WHERE user_id = ? AND business_type = "real_estate"', [userId], (err2, ownership) => {
                ...
                if (results.length > 0) {
                    ...
                    walletMoney = parseInt(results[0].wallet_money || 0);
...
app.post('/api/business/real-estate/add', (req, res) => {
    ...
    db.query('SELECT * FROM property_types WHERE id = ?', [typeId], (err, props) => {
        ...
        const prop = props[0];
        const costMoney = parseInt(prop.cost_money || 0);
...
app.post('/api/business/real-estate/withdraw', (req, res) => {
    ...
    db.query('SELECT real_estate_balance FROM user_business WHERE user_id = ?', [userId], (err, rows) => {
        ...
        let amount = parseInt(rows[0].real_estate_balance || 0);
        if (amount <= 0) return res.json({ success: false, message: 'Çekilecek bakiye yok.' });
